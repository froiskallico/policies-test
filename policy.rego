...
customer_data(customerUuid) := result if {
	customer := customers[customerUuid]
	result := customer
}

user_data(userUuid) := result if {
	customer := customers[input.customer]
...
check_unit_has_solution(unitUuid, actionUuid) := result if {
	unit := units[unitUuid]
	action_solution_uuid := actions[actionUuid].solution
...
check_unit_has_module(unitUuid, actionUuid) := result if {
	...
	action_module_uuid := actions[actionUuid].module
...
user_allowed_actions := [action |
	action := [uuid | uuid := key; _ := actions[key]][_]
